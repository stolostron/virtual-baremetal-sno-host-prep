...
while  IFS="," read cluster_name base_domain mac_addr ip_addr gateway machine_network_cidr public_ip_network_prefix bmc_addr bmc_username_base64 bmc_password_base64 ; do
	...
	sed -e s/cluster_name/$cluster_name/g \
	    -e s/public_key_base64/$public_key_base64/g \
	...
	sed -e s/cluster_name/$cluster_name/g \
            -e s/public_key_base64/$public_key_base64/g > $yaml_dir/800-infraenv.yaml
	...
	sed -e s/cluster_name/$cluster_name/g \
	    ...
	    -e s/cluster_name/$cluster_name/g > $yaml_dir/300-nmstate.yaml
	...
	echo -e "\n  $addonName:\n    enabled: $enabled" >> $yaml_dir/klusterletaddonconfig.yaml
    done
    #for row in $(jq '.acmAddonConfig[]' addon.json); do
    # jq -rc '.acmAddonConfig[]' addon.json | while IFS='' read stack;do
    # 	echo $stack
    # 	name=$(echo "$stack" | jq '.addonName')
    # 	echo $name
    # 	status=$(echo "$stack" | jq '.enabled')
    # 	echo $status
    # done
    
    # 	addon=`jq -r `
    # 	enabled=`jq '.acmAddonConfig[j].enabled' addon.json`
    # 	yq w $yaml_dir/klusterletaddonconfig.yaml '.spec.addon_array[j]' $enabled
    # done
